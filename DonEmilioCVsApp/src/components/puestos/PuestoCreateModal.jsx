...
export default function PuestoCreateModal({ show, onClose, onCreated, unidades }) {
  ...
  useEffect(() => {
    if (show) {
      setForm({ nombre: "", descripcion: "", unidad_id: "", activo: true });
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const created = await PuestosAPI.create(payload);
      toast.fire({ icon: "success", title: "Puesto creado" });
      onCreated?.(created);
      onClose?.();
    } catch (err) {
      toast.fire({ icon: "error", title: err.message || "No se pudo crear" });
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      <div className="bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 w-full max-w-lg overflow-hidden animate-[fadeIn_0.2s_ease-out]" ref={ref} onMouseDown={(e) => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-gray-900 to-gray-800">
          <h5 className="text-lg font-bold text-gray-100 m-0 font-lexend">Nuevo puesto</h5>
        ...
        <form onSubmit={onSubmit}>
          <div className="p-6 space-y-4">
            ...
            <div className="flex items-center pt-2 ml-1">
              <input id="pc-activo" className="h-4 w-4 text-brand-500 bg-gray-900 border-white/20 focus:ring-brand-500/50 rounded cursor-pointer transition-colors" type="checkbox" name="activo" checked={form.activo} onChange={onChange} />
              <label className="ml-2 block text-sm font-medium text-gray-300 cursor-pointer select-none" htmlFor="pc-activo">Activo</label>
            </div>
          </div>
